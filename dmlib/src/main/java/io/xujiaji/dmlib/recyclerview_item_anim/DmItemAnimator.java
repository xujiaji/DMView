...
public class DmItemAnimator extends SimpleItemAnimator {
    ...
    /**
     * 添加需要移除的item <br />
     * 返回true才会执行：{@link #runPendingAnimations()}
     */
    @Override
...
//        }
        if (newView != null) {
            ...
            newViewAnimation
                    .translationX(-newView.getRootView().getWidth())
    ...
    /**
     * 查找infoLList里面是否有这个item的改变动画，如果有则结束改变动画
     * @param infoList 所有item改变动画的信息
     * @param item 需要结束的item项
     */
    ...
    /**
     * 如果changeInfo有viewHolder对象，则结束该item的改变动画
     */
    ...
    /**
     * 判断是否结束改变动画<br />
     * 如果改变动画属于当前item的动画，则返回true
     * @param changeInfo 改变动画的信息
     * @param item 需要结束的
     * @return 是否结束改变动画
     */
    ...
    /**
     * 检测如果当前没有任何动画，则调用动画完成，结束父类中添加的动画监听
     */
     ...
     * {@inheritDoc}
     * <p/>
     * If the payload list is not empty, DmItemAnimator returns <code>true</code>.
     ...
     * then DmItemAnimator will call {@link #animateMove(ViewHolder, int, int, int, int)} and
